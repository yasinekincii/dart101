void main(List<String> args) {
sayilariTopla();

int fark= sayilariCikar(15,4);
  print("fark $fark");

//sayilariCarp(12, 6);
print("çarpım:" + sayilariCarp(12,6).toString());

print("Büyük olan sayi:"+maxOlaniBul(14,9).toString());
print("Küçük olan sayi:"+minOlanSayi(5,9).toString());
}



void sayilariTopla(){

int sayi1=10, sayi2=5;
print("Toplam: ${sayi1+sayi2}");
}

int sayilariCikar(int s1, int s2){
  return s1-s2 ; 
}

int sayilariCarp(int s1, int s2)=> s1*s2; 
//FAT ARROW(return ve süslü parantezleri kullanmıyoruz)(ok sonrası tek satır olmak zorunda)

/*
int maxOlaniBul(int s1, int s2){

  if (s1<s2) {
    return s2;
  }else{
    return s1;
  }
}
*/

int maxOlaniBul(int s1, int s2)=> s1<s2 ? s2 :s1;  

int minOlanSayi(int s1,int s2)=>s1<s2 ? s1 : s2;
